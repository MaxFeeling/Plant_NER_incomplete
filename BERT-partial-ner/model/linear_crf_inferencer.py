...
class LinearCRF(nn.Module):
    ...
    def get_prob(self, lstm_scores, annotation_mask, label_prob):
        batch_size = lstm_scores.size(0)
        seq_len = lstm_scores.size(1)
        prob = torch.zeros(batch_size, seq_len, self.label_size).to(self.device)
        if annotation_mask is not None:
            for idx in range(batch_size):
                for pos in range(seq_len):
                    if annotation_mask[idx, pos, :].min() == 1:
                        prob[idx, pos, 0] = label_prob[idx, pos, 0]
                        prob[idx, pos, 4] = label_prob[idx, pos, 1]
                        prob[idx, pos, 1:4] = label_prob[idx, pos, 2:5]/3
                        prob[idx, pos, 5:8] = label_prob[idx, pos, 2:5]/3
                        prob[idx, pos, 8:11] = label_prob[idx, pos, 2:5]/3
                        prob[idx, pos, 11:13] = label_prob[idx, pos, 5:7]
                        # prob[idx, pos, 11:13] = annotation_mask[idx, pos, 11:13]
                        # prob[idx, pos, 2:11] = (label_prob[idx, pos, 2:5] / 3).repeat(1, 3).view(9)

                    else:
                        prob[idx, pos, :] = annotation_mask[idx, pos, :]
            ...
            for idx in range(batch_size):
                for pos in range(seq_len):
                    ...
                    prob[idx, pos, 8:11] = label_prob[idx, pos, 2:5]/3
                    prob[idx, pos, 11:13] = label_prob[idx, pos, 5:7]
                    # prob[idx, pos, :2] = label_prob[idx, pos, :2]
                    # prob[idx, pos, 11] = label_prob[idx, pos, 5]
                    # prob[idx, pos, 12] = label_prob[idx, pos, 6]
                    #
                    # prob[idx, pos, 2:11] = (label_prob[idx, pos, 2:5] / 3).repeat(1, 3).view(9)
        prob = prob.float().log()
        return prob

    def forward(self, lstm_scores, word_seq_lens, tags, mask,label_prob,annotation_mask):
        """
        ...
        if label_prob is not None:
            prob = self.get_prob(lstm_scores=lstm_scores, label_prob=label_prob, annotation_mask=annotation_mask)
            all_scores=  self.calculate_all_scores(lstm_scores= lstm_scores,prob=None)
            entropy = self.entropy(label_prob)
            unlabed_score = self.forward_unlabeled(all_scores, word_seq_lens)

            labeled_score = self.forward_partial(all_scores,word_seq_lens,tags,mask,prob=prob)
            return unlabed_score, labeled_score, entropy
        else:
            all_scores = self.calculate_all_scores(lstm_scores=lstm_scores,prob=None)
            unlabed_score = self.forward_unlabeled(all_scores, word_seq_lens)
    ...
    def forward_partial(self, all_scores: torch.Tensor, word_seq_lens: torch.Tensor,tags:torch.Tensor,masks: torch.Tensor,prob:torch.Tensor) -> torch.Tensor:
        """
        ...
        alpha[:, 0, :] = all_scores[:, 0,  self.start_idx, :]+prob[:,0,:]## the first position of all labels = (the transition from start - > all labels) + current emission.

        # alpha[:, 0, :] = alpha[:, 0, :].mul(masks[:,0,:])
        for word_idx in range(1, seq_len):
            ...
            before_log_sum_exp = alpha[:, word_idx-1, :].view(batch_size, self.label_size, 1).expand(batch_size, self.label_size, self.label_size) + all_scores[:,word_idx,:,:]
                                 # pre_mask.view(batch_size,self.label_size,1).expand(batch_size,self.label_size,self.label_size)
            # before_log_sum_exp[before_log_sum_exp==-float("Inf")]=-10000.0
                #防止溢出，变成Na
            # before_log_sum_exp = before_log_sum_exp.mul(pre_mask.view(batch_size,self.label_size,1).expand(batch_size,self.label_size,self.label_size))
            # before_log_sum_exp = before_log_sum_exp.mul(cur_mask.view(batch_size,1,self.label_size).expand(batch_size,self.label_size,self.label_size))
            alpha[:,word_idx,:] = log_sum_exp_pytorch(before_log_sum_exp)+prob[:,word_idx,:]
            # alpha[:,word_idx,:] = alpha_next.mul(cur_mask)
    ...
    def calculate_all_scores(self, lstm_scores: torch.Tensor,prob:torch.Tensor) -> torch.Tensor:
        """
        ...
        if prob is not None:
            scores += prob.view(batch_size,seq_len,1,self.label_size).expand(batch_size,seq_len,self.label_size,self.label_size)
        return scores

    def decode(self, features, wordSeqLengths, annotation_mask,label_prob) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        ...
        """
        if label_prob is not None:
            prob = self.get_prob(features, annotation_mask=None, label_prob=label_prob)

            all_scores = self.calculate_all_scores(features,prob=prob)
        else:
            all_scores = self.calculate_all_scores(features,prob=None)
    ...
    def entropy(self,prob):
        loss = (-prob).mul(torch.log(prob))
        sum_loss = torch.sum(torch.sum(loss,1))
        return sum_loss