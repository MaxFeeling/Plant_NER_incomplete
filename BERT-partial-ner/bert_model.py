...
class BertCRF(BertPreTrainedModel):
    ...
    def forward(self, input_ids, input_seq_lens=None, annotation_mask=None, labels=None,
                ...
                position_ids=None, head_mask=None, label_prob =None):
        ...
        if labels is not None:
            ...
            if label_prob is not None:
                unlabed_score, labeled_score,entropy = self.inferencer(logits, input_seq_lens, labels, attention_mask,label_prob,annotation_mask)
                return unlabed_score - labeled_score+entropy
            else:
                unlabed_score, labeled_score = self.inferencer(logits, input_seq_lens, labels, attention_mask,label_prob, annotation_mask)
                return unlabed_score-labeled_score
        else:
            bestScores, decodeIdx = self.inferencer.decode(logits, input_seq_lens, annotation_mask,label_prob)

            return bestScores, decodeIdx

    # obsolete
    def decode(self, input_ids, input_seq_lens=None, annotation_mask=None, attention_mask=None,label_prob=None) -> Tuple[torch.Tensor, torch.Tensor]:
        """
